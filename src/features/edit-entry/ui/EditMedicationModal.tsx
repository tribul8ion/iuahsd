...
import {
  ...
  DEFAULT_TIMES, INTERVAL_DEFAULT, MED_INTERVAL_MAX, clampInterval, parseDosageAmount, isColorKey,
} from "@/entities/entry";
import type { Entry, EntrySchedule, FrequencyType, DosageUnit, ColorKey } from "@/entities/entry";
...
function formatDosageInitial(amount: number | null): string {
  return amount === null || amount === undefined ? "" : String(amount);
}

export function EditMedicationModal({ medication, isOpen, onClose, onDelete }: EditMedicationModalProps) {
  const { t } = useTranslation();
  const { mutate, isPending } = useEditEntry();

  const [name, setName] = useState(medication.name);
  const [schedule, setSchedule] = useState<EntrySchedule>(medication.schedule);
  const [times, setTimes] = useState<Record<EntrySchedule, string>>({ ...DEFAULT_TIMES, [medication.schedule]: medication.time });
  const [editingTimeFor, setEditingTimeFor] = useState<EntrySchedule | null>(null);
  const [frequencyType, setFrequencyType] = useState<FrequencyType>(medication.frequency_type ?? "daily");
  const [intervalDays, setIntervalDays] = useState<number>(medication.interval_days ?? INTERVAL_DEFAULT);
  const [startDate, setStartDate] = useState<string>(medication.start_date ?? todayISO());
  const [dosageAmountRaw, setDosageAmountRaw] = useState<string>(formatDosageInitial(medication.doseAmount));
  const [dosageUnit, setDosageUnit] = useState<DosageUnit>(medication.doseUnit ?? "tablet");
  const [isUnitModalOpen, setIsUnitModalOpen] = useState(false);
  const [color, setColor] = useState<ColorKey | null>(
    isColorKey(medication.color) ? medication.color : null
  );
  const [preRemind, setPreRemind] = useState<number[]>(medication.pre_remind ?? []);
  ...
  const handleSubmit = useCallback(() => {
    ...
    name, schedule, times, frequencyType, intervalDays, startDate, dosageAmountRaw, dosageUnit, color, preRemind,
    medication.id, medication.notes, medication.active, mutate, onClose,
  ]);
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <FrequencyPicker
                ...
                startDate={{ label: t("medications.frequency_start_date"), value: startDate, onChange: setStartDate }}
              />
            </div>
